...
export function listItemToArgument(node: Remark.ListItemNode): ResourceArgument {
  ...
  const argumentName: string = get(node, ['children', 0, 'children', 0, 'value']);
 ...
 */
export default function extractArguments(ast: Remark.RootNode): {[argName: string]: ResourceArgument} {
  ...
  for (let i = 0; i < ast.children.length; i++) {
    ...
    switch (currentChild.type) {
      case 'heading':
        if (get(currentChild, ['children', 0, 'value'], '').trim().toLowerCase() === ARGREF_HEADER) {
          foundHeading = true;
        }
        break;
      case 'list':
        if (!foundHeading) break;
        return (currentChild as Remark.ListNode).children.map(listItemToArgument)
          .reduce(
            (args, next: ResourceArgument) => ({...args, [next.argumentName]: next}),
            {}
          );
      default: break;
    }

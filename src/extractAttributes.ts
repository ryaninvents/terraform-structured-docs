...
function listItemToAttribute(node: Remark.ListNode): ResourceAttribute {
  const attributeName: string = get(node, ['children', 0, 'children', 0, 'value']);
  ...
  return {
    attributeName,
    description: unified().use(stringify)
      .stringify(newNode)
 ...
 */
export default function extractAttributes(ast: Remark.RootNode): {[argName: string]: ResourceAttribute} {
  ...
  for (let i = 0; i < ast.children.length; i++) {
    ...
    switch (currentChild.type) {
      case 'heading':
        if (get(currentChild, ['children', 0, 'value'], '').trim().toLowerCase() === ATTRREF_HEADER) {
          foundHeading = true;
        }
        break;
      case 'list':
        if (!foundHeading) break;
        return (currentChild as Remark.ListNode).children.map(listItemToAttribute)
          .reduce(
            (args, next: ResourceAttribute) => ({...args, [next.attributeName]: next}),
            {}
          );
      default: break;
    }
